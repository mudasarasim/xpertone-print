...
const CartSidebar = () => {
  ...
  const subtotal = cartItems.reduce((acc, item) => {
    const price = Number(item.quantity_500 || 0);
                  ...
                  return (
                    <li key={item.id}>
                      ...
                      <div className="ms-pro-content">
                        ...
                        {item.category && <div><small className="text-muted">Category: {item.category}</small></div>}
                        {item.dimensions && <div><small className="text-muted">Size: {item.dimensions}</small></div>}
